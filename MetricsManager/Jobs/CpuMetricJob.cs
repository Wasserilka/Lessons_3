...
namespace MetricsManager.Jobs
{
    ...
    public class CpuMetricJob : IJob
    {
        ...
        public CpuMetricJob(ICpuMetricsRepository repository, IAgentsRepository agentsRepository,
        ...
        public Task Execute(IJobExecutionContext context)
        {
            ...
            foreach (AgentInfo agent in agents)
            {
                ...
                var metricResponse = _agentClient.GetCpuMetrics(
                    new CpuMetricApiRequest { FromTime = fromTime, ToTime = toTime, AgentUrl = agent.AgentUrl });

                if (metricResponse != null)
                {
                    foreach (var metric in metricResponse.metrics)
                    {
                        _repository.Create(_mapper.Map<CpuMetric>(metric), agent.AgentId);
                    }
                }
            }
